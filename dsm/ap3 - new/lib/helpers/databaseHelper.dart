...
class DatabaseHelper{
  ...
  Future<Database> initializeDatabase() async {
    ...
    String path = directory.path + 'contatosv2.db';
  ...
  void _createDb(Database db, int newVersion) async {
    await db.execute('CREATE TABLE $contatoTable($colId INTEGER PRIMARY KEY AUTOINCREMENT, '
                     '$colNome TEXT, $colEmail TEXT, $colImagem TEXT, $colRua TEXT, $colBairro TEXT, $colCep TEXT)');  
  }
  ...
  Future<Contato> getContato(int id) async {
    ...
    List<Map> maps = await db.query(contatoTable,
                                    columns: [colId, colNome, colEmail, colImagem, colRua, colBairro, colCep],
                                    where: "$colId = ?",
                                    whereArgs: [id] );
